...
<!-- Hero Section -->
	<section class="relative h-[600px] lg:h-[876px] flex items-center overflow-hidden">
		<!-- Background Image -->
		...
		<div class="relative max-w-[1200px] mx-auto px-4 w-full">
			<div class="max-w-2xl">
				<h1 class="text-lg font-medium text-[#424242] mb-6 leading-[1.2] lg:-mt-24">
	...
	<!-- Basic Directions -->
	<section class="py-16 lg:py-8 lg:pb-15 dark:bg-gray-900 bg-[url('<?php the_field(
    	'sec2-bg',
	) ?>')] bg-cover bg-center bg-no-repeat basic-directions-section">
		<div class="max-w-[1200px] mx-auto px-4">
			<div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-12 gap-4">
				<h2 class="text-[26px] lg:text-[44px] font-bold text-[#222222]">Основные направления</h2>
				<?php
				...
				<a href="<?php echo
    				esc_url($catalog_url)
				; ?>" class="basic-directions-btn inline-flex items-center gap-2 bg-white text-[#1e3a5f] border border-[#D0D6E8] px-8 py-3 rounded-sm font-medium hover:bg-gray-50 transition-colors">
					Перейти в каталог
				</a>
			</div>
				...
				if (!is_wp_error($categories) && !empty($categories)) {
    				foreach ($categories as $category) {
        				...
        				$description = !empty($category->description)
            				...
            				: 'Категория товаров для вашего склада.';
						...
						<div class="bg-white border overflow-hidden shadow-sm hover:shadow-lg transition-shadow group">
							<div class="h-[250px] bg-gray-100 overflow-hidden">
								<img src="<?php echo esc_url($image_url); ?>" alt="<?php echo
	...
	<!-- Services -->
	<section class="py-16 lg:py-24 bg-[#f8fafc] services-section bg-[url('<?php the_field(
    	'sec3-bg',
	) ?>')] bg-cover bg-center bg-no-repeat">
		<div class="max-w-[1200px] mx-auto px-4">
			<h2 class="text-[26px] lg:text-[44px] font-bold text-[#222222] mb-12"><?php the_field('sec3_heading') ?></h2>
    				...
    				while ($services_query->have_posts()) {
						...
						<div class="bg-white dark:bg-gray-800 relative min-h-[226px] p-[20px] shadow-sm hover:shadow-md transition-shadow">
							<div class="flex items-center">
								<div class="w-[58px] h-[44px] flex items-center justify-center mb-4 text-[#1e3a5f] dark:text-gray-100 mr-[10px]">
									<?php if ($icon_url): ?>
										<img src="<?php echo esc_url($icon_url); ?>" alt="<?php echo
    										esc_attr($icon_alt)
										; ?>" class="w-[58px] h-[44px]">
									<?php else: ?>
							...
							<p class="text-base font-normal leading-[1.2] text-gray-600 dark:text-gray-300 mb-4"><?php echo
    							esc_html($service_desc)
							; ?></p>
							<a href="<?php echo
    							esc_url($service_permalink)
							; ?>" class="absolute bottom-[20px] basic-directions-btn bg-white border border-[#D0D6E8] rounded-sm hover:text-white text-[#1e3a5f] px-[20px] py-[5px] font-medium text-sm hover:bg-[#222222] inline-flex items-center gap-1">
								Подробнее
							</a>
						</div>
	...
	<!-- About Section -->
	<section class="py-16 lg:py-24 bg-white dark:bg-gray-900 bg-[url('<?php the_field(
    	'sec4-bg',
	) ?>')] bg-cover bg-center bg-no-repeat about-section">
				...
				<div class="relative">
					<div class="overflow-hidden shadow-lg mb-6">
						<img src="<?php the_field(
    						'sec4_img-right',
						) ?>" alt="Robot" class="w-full h-auto md:min-h-[472px] md:min-w-[588px]">
					</div>
	...
	<!-- Projects Section -->
	<section class="py-16 lg:py-24 bg-[#f8fafc] dark:bg-gray-900 bg-[url('<?php the_field(
    	'sec5_bg',
	) ?>')] bg-cover bg-center bg-no-repeat projects-section">
		<div class="max-w-[1200px] mx-auto px-4">
			<div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-12 gap-4">
				<h2 class="text-[26px] lg:text-[44px] font-bold text-[#222222] dark:text-gray-100"><?php the_field(
    				'sec5_heading',
				); ?></h2>
				<?php
	...
	<!-- News Section -->
	<section class="py-16 lg:py-24 bg-white dark:bg-gray-900 news-section bg-[url('<?php the_field(
    	'sec6_bg',
	) ?>')] bg-cover bg-center bg-no-repeat">
		<div class="max-w-[1200px] mx-auto px-4">
<div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-12 gap-4">
    				...
    				while ($news_query->have_posts()) {
						...
						<a href="<?php the_permalink(); ?>" class="group cursor-pointer block bg-white dark:bg-gray-800 shadow">
							...
							<div class="flex items-center p-4">
								<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#A6AAB4" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" data-lucide="calendar" aria-hidden="true" class="lucide lucide-calendar h-4 w-4"><path d="M8 2v4"></path><path d="M16 2v4"></path><rect width="18" height="18" x="3" y="4" rx="2"></rect><path d="M3 10h18"></path></svg>
							<p class="text-sm ml-1 text-[#A6AAB4] text-light dark:text-gray-400"><?php echo
