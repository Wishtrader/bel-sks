...
?>
  ...
  <div class="relative mx-auto w-full max-w-[1200px] px-4 sm:px-6 lg:px-0 lg:mt-10">
    <nav class="mb-6 text-[14px] leading-none text-slate-500 sm:mb-8">
      ...
      <span class="text-[#294F78]">Каталог</span>
    </nav>

    <h1 class="mb-8 lg:mt-18 text-[26px] lg:!text-[44px] font-extrabold tracking-[0.5px] text-[#222222] sm:mb-10">
      Каталог
    </h1>
    			...
    			foreach ($parent_categories as $parent_cat) {
					...
					<li class="catalog-category">
						<div class="flex items-center justify-between gap-2">
							<a href="<?php echo
    							esc_url($parent_link)
							; ?>" class="flex-1 text-[18px] font-semibold leading-snug text-[#3f5d7e] transition-colors hover:text-slate-900">
								<?php echo esc_html($parent_cat->name); ?>
							</a>
							<?php if ($has_children): ?>
								<button type="button" class="catalog-toggle shrink-0 p-1 text-[#3f5d7e] transition-colors hover:text-slate-900" aria-expanded="false" aria-label="<?php esc_attr_e(
    								'Раскрыть подкатегории',
    								'belsks',
								); ?>">
					  <img src="<?php echo
    					  get_template_directory_uri()
					  ; ?>/img/arrow-down.svg" class="catalog-chevron h-5 w-5 transition-transform duration-200" />
								</button>
							<?php endif; ?>
						</div>
						<?php if ($has_children): ?>
							<ul class="catalog-subcategories hidden mt-3 ml-2 space-y-2 border-l border-slate-200 pl-4">
								<?php foreach ($children as $child_cat): ?>
									...
									<li>
										<a href="<?php echo
    										esc_url($child_link)
										; ?>" class="block text-[15px] leading-snug text-slate-600 transition-colors hover:text-[#3f5d7e]">
				...
				<article class="overflow-hidden bg-white shadow-[0_2px_12px_rgba(15,23,42,0.08)]">
					...
					<div class="p-5">
						<h2 class="mb-3 text-[18px] font-bold leading-snug text-slate-900">
							<a href="<?php echo esc_url($category_link); ?>" class="hover:text-[#3d5f86]"><?php echo
