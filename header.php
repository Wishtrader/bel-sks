	...
	<header id="masthead" class="site-header shadow-md fixed top-0 left-0 right-0 z-50">
		<div class="max-w-[1200px] mx-auto px-4">
			<div class="flex items-center gap-8">
				...
				<!-- Right side: Two rows -->
				<div class="flex-1 hidden min-[1080px]:flex flex-col">
					...
					<div class="flex items-center justify-between py-3">
						...
						<div class="flex items-center gap-6">
							...
							<div class="flex items-center gap-4">
								<a href="tel:+375172381717" class="flex items-center gap-1 text-sm text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors whitespace-nowrap">
									<?php
									...
									echo $phone_icon;
									?>
									<span class="whitespace-nowrap shrink-0">+375 17 238 17 17</span>
								</a>
								<a href="tel:+375173748682" class="flex items-center gap-1 text-sm text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors whitespace-nowrap">
									<?php
									...
									echo $phone_icon;
									?>
									<span class="whitespace-nowrap shrink-0">+375 17 374 86 82</span>
								</a>
								<a href="tel:+375447797030" class="flex items-center gap-1 text-sm text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors whitespace-nowrap">
									<?php
									...
									echo $phone_icon;
									?>
									<span class="whitespace-nowrap shrink-0">+375 44 779 70 30</span>
								</a>
							</div>
						...
						<!-- Language switcher -->
						<div class="gtranslate_wrapper text-sm">
				...
				<nav id="site-navigation" class="main-navigation flex items-center justify-between gap-8 py-3">
					...
					<?php if (class_exists('WooCommerce')): ?>
						...
						<a href="<?php echo
    						esc_url($cart_url)
						; ?>" class="relative ml-auto inline-flex items-center text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors <?php echo
    						$cart_count > 0 ? '' : 'hidden'
						; ?>" aria-label="<?php esc_attr_e('Корзина', 'belsks'); ?>" data-cart-link>
						<img src="<?php echo get_template_directory_uri(); ?>/img/cart.svg" alt="cart-icon" class="w-6 h-6" />
							<span data-cart-count class="<?php echo
    							$cart_count > 0 ? '' : 'hidden'
							; ?> absolute -top-2 -right-3 min-w-[20px] h-[20px] px-1 bg-white inline-flex items-center justify-center rounded-full border-[1px] border-gray-300 text-white !text-[9px] font-bold leading-none"><?php echo
    							esc_html($cart_count)
							; ?></span>
						</a>
					<?php endif; ?>
				...
				<div class="flex min-[1080px]:hidden items-center gap-3 flex-1">
			...
			<!-- Mobile menu -->
			<div id="mobile-menu" class="hidden min-[1080px]:hidden absolute left-0 right-0 top-full bg-white dark:bg-gray-900 shadow-lg z-40 max-h-[calc(100vh-78px)] overflow-y-auto">
				<div class="px-4 py-6">
					<!-- Navigation -->
					<nav class="mb-6">
						<?php
					...
					<div class="space-y-3 mb-6 pb-6 border-b border-gray-200 dark:border-gray-700">
						<a href="tel:+375172381717" class="flex items-center gap-2 text-[#222222] dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors whitespace-nowrap">
							<?php
						...
						<a href="tel:+375173748682" class="flex items-center gap-2 text-[#222222] dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors whitespace-nowrap">
							<?php
						...
						<a href="tel:+375447797030" class="flex items-center gap-2 text-[#222222] dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors whitespace-nowrap">
							<?php
...
<!-- Language switcher -->
							...
							<?php if (class_exists('WooCommerce')): ?>
								...
								$cart_url_m = function_exists('wc_get_cart_url') ? wc_get_cart_url() : '#';
								$cart_count_m = function_exists('WC') && WC()->cart ? (int) WC()->cart->get_cart_contents_count() : 0;
								?>
								<a href="<?php echo
    								esc_url($cart_url_m)
								; ?>" class="relative inline-flex items-center text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors <?php echo
    								$cart_count_m > 0 ? '' : 'hidden'
								; ?>" aria-label="<?php esc_attr_e('Корзина', 'belsks'); ?>" data-cart-link>
									<i data-lucide="shopping-cart" class="w-6 h-6"></i>
									<span data-cart-count class="<?php echo
    									$cart_count_m > 0 ? '' : 'hidden'
									; ?> absolute -top-2 -right-3 min-w-[20px] h-[20px] px-1 inline-flex items-center justify-center rounded-full text-white text-[11px] font-bold leading-none"><?php echo
    									esc_html($cart_count_m)
									; ?></span>
								</a>
							<?php endif; ?>
					</div>
