...
<main class="bg-[#f7f7fb]">
	...
	<section class="relative overflow-hidden bg-[#f7f7fb] py-10 sm:py-14 lg:py-16">
		...
		<div class="relative mx-auto w-full max-w-[1200px] px-4 sm:px-6 lg:px-4">
			<nav class="mb-6 text-[14px] leading-none text-slate-500 sm:mb-8 lg:mt-10">
				...
				<?php foreach ($ancestors as $ancestor_id): ?>
					...
					<?php if ($ancestor && !is_wp_error($ancestor)): ?>
						...
						; ?>" class="transition-colors hover:text-slate-700"><?php echo esc_html($ancestor->name); ?></a>
					<?php endif; ?>
				<?php endforeach; ?>
				<span class="mx-1 text-slate-400">/</span>
				<span class="text-[#294F78]"><?php echo esc_html($current_term->name); ?></span>
			</nav>

			<h1 class="mb-4 !text-[26px] lg:!text-[44px] lg:mt-17 !font-bold leading-[1.2] tracking-[0px] text-[#222222] sm:mb-6 sm:text-[56px]">
			...
			<div class="grid gap-6 lg:grid-cols-[380px_minmax(0,1fr)] lg:items-start">
				<aside class="rounded-sm border border-slate-200 bg-white p-4 shadow-[0_2px_12px_rgba(15,23,42,0.08)] sm:p-6">
					<div class="mb-4 inline-flex rounded-md border border-slate-300 px-5 py-2 text-[18px] font-semibold leading-none text-[#3f5d7e]">
						Категории
					</div>
    						...
    						foreach ($parent_categories as $parent_cat) {
								...
								<li class="catalog-category">
									<div class="flex items-center justify-between gap-2">
										<a href="<?php echo esc_url($parent_link); ?>" class="flex-1 rounded-md px-3 py-2 text-[18px] font-semibold leading-snug <?php echo
    										$is_current_term ? 'bg-[#3d5f86] text-white' : 'text-[#3f5d7e]'
										; ?> transition-colors hover:text-slate-900">
											<?php echo esc_html($parent_cat->name); ?>
										</a>
										<?php if ($has_sub_children): ?>
											<button type="button" class="catalog-toggle shrink-0 p-1 text-[#3f5d7e] transition-colors hover:text-slate-900" aria-expanded="<?php echo
    											$should_be_open ? 'true' : 'false'
											; ?>" aria-label="<?php esc_attr_e('Раскрыть подкатегории', 'belsks'); ?>">
														  <img src="<?php echo
    														  get_template_directory_uri()
														  ; ?>/img/arrow-down.svg" class="catalog-chevron h-5 w-5 transition-transform duration-200 <?php echo $should_be_open ? 'rotate-180' : ''; ?>" />
											</button>
										<?php endif; ?>
									</div>
									<?php if ($has_sub_children): ?>
										<ul class="catalog-subcategories <?php echo
    										$should_be_open ? '' : 'hidden'
										; ?> mt-3 ml-2 space-y-2 border-l border-slate-200 pl-4">
											<?php foreach ($sub_children as $sub_child): ?>
												...
												?>
												<li>
													<a href="<?php echo esc_url($sub_child_link); ?>" class="block rounded-md px-3 py-1 text-[15px] leading-snug <?php echo
    													$is_current_sub_child ? 'bg-[#3d5f86] font-semibold text-white' : 'text-slate-600'
													; ?> transition-colors hover:text-[#3f5d7e]">
				...
				<div class="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
					<?php if ($has_child_categories): ?>
						...
						foreach ($child_categories as $child_cat) {
    						...
    						$description = !empty($child_cat->description)
							...
							<article class="overflow-hidden bg-white shadow-[0_2px_12px_rgba(15,23,42,0.08)]">
								<div class="h-[160px] bg-slate-100 flex items-center justify-center overflow-hidden">
									<img src="<?php echo esc_url($image_url); ?>" alt="<?php echo
    									esc_attr($child_cat->name)
									; ?>" class="w-full h-full object-cover">
								</div>
								<div class="p-5">
									<h2 class="text-[18px] font-bold leading-snug text-slate-900">
										<a href="<?php echo esc_url($child_link); ?>" class="hover:text-[#3d5f86]"><?php echo
					...
					<?php else: ?>
						...
						if ($products->have_posts()):
    						while ($products->have_posts()):
								...
								<article class="overflow-hidden bg-white shadow-[0_2px_12px_rgba(15,23,42,0.08)]">
									<div class="h-[160px] bg-slate-100 flex items-center justify-center overflow-hidden">
										<img src="<?php echo esc_url($product_image); ?>" alt="<?php echo
